# Chapter 21: The Fleet

> [!IMPORTANT] > **Tone Rule:** All summaries must be written in a **Neutral and Factual** tone. Avoid subjective commentary or interpretation—focus on what happens, who is involved, and what is said.

> [!NOTE]
> There is **no limit** to the number of scenes, key events, character defining moments, or key dialog entries. Include as many as necessary to fully capture the chapter.

---

### Scene 1: The Bridge

- **Key Event:** The survivors return to the bridge to find the alien fleet has decloaked, surrounding the Zenith with thousands of "Needle-Ships" and a massive Mothership.

  - _Character Defining Moment:_ **General Ironwood** — Experiences a moment of genuine awe and despair, realizing the scale of the threat and the obsolescence of human technology.
  - _Key Dialog:_ **General Ironwood:** "It's not a fleet. It's a harvest."
  - _Key Dialog:_ **AI:** "Threat Level: Extinction. Strategic options: Zero."

- **Key Event:** Vane reveals the existence of the "Star-Tear Drive," a hidden jump drive that requires a human pilot to act as a CPU, specifically someone with the Sangreal bloodline.

  - _Character Defining Moment:_ **Prelate Vane** — Reveals his knowledge of the ship's secret purpose and the true nature of Angelica's DNA.
  - _Key Dialog:_ **Prelate Vane:** "The ship is a body, Angelica. And it's been waiting for its brain."
  - _Key Dialog:_ **Angelica Ward:** "I'm not a brain. I'm a person."

_Copy this template for each scene. Add or remove scenes and events as needed._


